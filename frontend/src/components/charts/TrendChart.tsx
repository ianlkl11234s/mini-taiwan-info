/**
 * TrendChart — 多 series 折線圖（responsive width）
 * 對應 prototype trend.jsx
 */
    ...
    <div ref={ref} className="chart-wrap" style={{ width: "100%", height, position: "relative" }}>
      <svg width={w} height={height} className="chart-svg">
        ...
        <g className="chart-axis">
          {gridLines.map((g, i) => (
            <text key={i} x={padding.l - 8} y={g.y + 3} textAnchor="end">
              {Math.abs(g.v) >= 1000
                ? fmt.num(Math.round(g.v))
                : g.v.toFixed(g.v >= 100 || g.v <= -100 ? 0 : 1)}
            </text>
          ))}
          {xLabels && (
            <g>
        ...
        {normalized.map((s, sI) => {
          ...
          const areaPath =
            linePath +
            ` L ${pts[pts.length - 1]?.[0] ?? 0} ${padding.t + innerH} L ${pts[0]?.[0] ?? 0} ${padding.t + innerH} Z`;
          return (
            <g key={sI}>
