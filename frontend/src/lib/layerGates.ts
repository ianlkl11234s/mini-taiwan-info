...
/**
 * 動態圖層鎖定（自 mini-taiwan-pulse 移植；同一 Supabase project 共用
 * gated_layers / get_layer_gates() / enforce_layer_access()）。
 *
 * 公開 RPC `get_layer_gates()`（anon 可呼叫，只回 keys 不含地理資料）提供
 * 「哪些 layer 被鎖 + 需要什麼 tier」的權威清單。
 *
 * fail-safe：RPC 失敗 / 尚未回來時 gatesCache=null → 一律 fallback 回靜態
 * STATIC_GATED_LAYERS（保持鎖定，絕不因 RPC 失敗而解鎖）。
 *
 * migration 281 前：gates 內沒有 info_medical_ltc 這列 → 該層照舊公開；
 * apply 後：出現該列（required_tier=member, lock_type=full）→ 前端上鎖 +
 * DB 端 enforce（anon 401/403、tier 不足 42501）。
 */
...
/** 靜態 fail-safe fallback：get_layer_gates() 失敗時這些 key 保持上鎖 */
const STATIC_GATED_LAYERS: ReadonlySet<string> = new Set(["info_medical_ltc"]);
...
/** App 端往下傳的閘門 context（useMemberGate 的 tier + useLayerGates 的 gates） */
...
export async function loadLayerGates(): Promise<void> {
  ...
  try {
    ...
    if (import.meta.env.DEV) console.warn("[layerGates] load failed → fallback to static STATIC_GATED_LAYERS", err);
  } finally {
    inflight = false;
  }
}
 ...
 * - gates=null（未載入 / 失敗）→ fail-safe 用靜態 STATIC_GATED_LAYERS，一律當 full 鎖需 owner。
 */
export function isLayerLocked(
  ...
  return STATIC_GATED_LAYERS.has(key) && userRank < tierRank("owner");
}

/** 後端鎖定 RPC 對非授權者回 401/403 / code 42501 —— 視為「無權限」靜默處理（不噴 error / 不重試）。 */
