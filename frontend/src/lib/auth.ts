...
/**
 * 會員 Auth 薄封裝（自 mini-taiwan-pulse 移植，同一 Supabase project 共用
 * auth.users / profiles）。
 * Google OAuth only；session 持久化 / 自動刷新 / URL detect 皆走 supabase.ts
 * 主 client 的 supabase-js v2 預設（persistSession / autoRefreshToken / detectSessionInUrl）。
 */
...
/**
 * 會員分級閘門。
 * 登入後 select profiles.tier where id = auth.uid()（RLS 允許讀自己 row）。
 * - tier 載入為非同步：載入完成前 tier=null（顯示鎖），完成後才可能解鎖。
 * - 未登入：user=null / tier=null / isOwner=false。
 * - tier 用於分層 gating（free/member/insider/owner，見 lib/layerGates）。
 */
